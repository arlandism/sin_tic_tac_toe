...
describe GameHistoryPresenter do
  ...
  it "creates headers with game ids" do
    ...
    GameHistoryPresenter.show_games.should == game_header % [id] + game_footer + game_header % [id_two] + game_footer
  end
  ...
  it "lists the moves for each game in order" do
    ...
    GameHistoryPresenter.show_games.should == game_header % [id] +  "<ol><li>x moved to 3</li></ol>" + game_footer
  end

  it "lists the winner for each game" do
    pre_conversion = {"games" => {id => {"winner" => "x"}}}
    GameRecorder.stub(:compute_file_contents).and_return(pre_conversion)
    GameHistoryPresenter.show_games.should == game_header % [id] + "X won<br /><br />" + game_footer
  end
