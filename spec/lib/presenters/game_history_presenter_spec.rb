...
describe GameHistoryPresenter do
  ...
  it "creates headers with game ids" do
    id_two = 1800
    GameRecorder.stub(:compute_file_contents).with(path).and_return({"games" => {id => {}, id_two => {}}})
  ...
  it "lists the moves for each game in order" do
    ...
    GameRecorder.stub(:compute_file_contents).with(path).and_return(pre_conversion)
